...
interface EventReviewsProps {
  reviews: Review[];
  onAddReview: () => void;
  averageRating: number;  // A média deve ser calculada no componente pai
}
...
export function EventReviews({ reviews = [], onAddReview, averageRating }: EventReviewsProps) {
  ...
  return (
    <Box mt={20} mb={4} display="flex" flexDirection="column">
      ...
      <Box mt={8} width="100%">
        {/* Exibir até 2 avaliações, se houver */}
        {reviews.length > 0 ? (
          reviews.slice(0, 2).map((review, index) => {
            console.log("Foto do usuário:", review.userPhoto);  // Verifique o valor aqui
